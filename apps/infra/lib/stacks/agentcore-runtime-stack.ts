...
export class AgentCoreRuntimeStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: AgentCoreRuntimeStackProps) {
    ...
    // Get Aurora cluster endpoint for RAG
    const clusterEndpoint = ssm.StringParameter.valueForStringParameter(
      this,
      `${paramPrefix}/Aurora/ClusterEndpoint`
    );

    const databaseName = ssm.StringParameter.valueForStringParameter(
      this,
      `${paramPrefix}/Aurora/DatabaseName`
    );

    const vectorTableName = ssm.StringParameter.valueForStringParameter(
      this,
      `${paramPrefix}/Aurora/VectorTableName`
    );

    const secretArn = ssm.StringParameter.valueForStringParameter(
      this,
      `${paramPrefix}/Aurora/SecretArn`
    );
    ...
    // Note: Using IAM authorization instead of Cognito
    // SDK invocation only supports IAM/SigV4 authentication
    // For Cognito/OAuth, direct HTTPS requests would be required
    ...
    // Grant permissions to access Aurora RDS
    this.runtime.addToRolePolicy(
      new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
        actions: [
    ...
    // Grant permissions to read Aurora database credentials
    this.runtime.addToRolePolicy(
      new iam.PolicyStatement({
        effect: iam.Effect.ALLOW,
        actions: [
