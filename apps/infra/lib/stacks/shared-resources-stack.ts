...
export class SharedResourcesStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props?: SharedResourcesStackProps) {
    ...
    this.commonLayer = new lambda.LayerVersion(this, 'CommonLayer', {
      ...
      code: lambda.Code.fromAsset(path.join(__dirname, '../../../../packages/lambdas/layers/common')),
    ...
    this.bedrockAccessPolicy = new iam.ManagedPolicy(this, 'BedrockAccessPolicy', {
      ...
      statements: [
        // Foundation model invocations (require wildcard for cross-region inference)
        ...
        // Agent operations (scoped to account)
        new iam.PolicyStatement({
          effect: iam.Effect.ALLOW,
          actions: [
            'bedrock:InvokeAgent',
        ...
        // Knowledge base operations (scoped to account)
        new iam.PolicyStatement({
          effect: iam.Effect.ALLOW,
          actions: [
