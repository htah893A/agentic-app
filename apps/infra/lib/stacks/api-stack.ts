...
export class ApiStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: ApiStackProps) {
    ...
    // Create API key for usage plan tracking (auto-generated, not hardcoded)
    ...
    // Create a shared request validator for all methods
    ...
    this.chatFunction = new NodejsFunction(this, 'ChatFunction', {
      ...
      handler: 'handler',
      entry: path.join(__dirname, '../../../../packages/lambdas/chat/src/index.ts'),
      environment: {
      ...
      // Removed reserved concurrency to avoid account limits during deployment
      bundling: {
        minify: true,
        sourceMap: true,
    ...
    healthResource.addMethod('GET', new apigateway.LambdaIntegration(healthCheckLambda), {
      ...
      // No auth intentionally — health check must be accessible for monitoring/load balancers
      requestValidator,
    });
