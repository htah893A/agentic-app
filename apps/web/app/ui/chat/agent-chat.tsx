...
function Message({
  ...
  return (
    <div
      className={`mb-4 flex animate-fadeSlideIn flex-col ${isUser ? 'items-end' : 'items-start'}`}
    >
...
function EmptyState({ onPick }: { onPick: (text: string) => void }) {
  const suggestions = [
    'Explain my architecture options',
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-6 p-8">
      <div className="text-center">
        <div className="mx-auto mb-3 flex h-10 w-10 items-center justify-center rounded-full border border-chat-border bg-chat-surface">
          <svg
            width="18"
            height="18"
            viewBox="0 0 24 24"
        ...
        <p className="text-[15px] font-medium text-neutral-900 dark:text-neutral-100">How can I help?</p>
        <p className="mt-1 text-sm text-neutral-500">Ask anything or try a suggestion below</p>
      </div>
...
function recordsToMessages(items: Record<string, unknown>[]): Msg[] {
  ...
  for (const item of items) {
    ...
    const content =
      unwrapDdb(item.content) ||
      unwrapDdb(item.message) ||
      unwrapDdb(item.text) ||
      unwrapDdb(item.body);
...
export default function AgentChat({ initialSessionId }: { initialSessionId?: string }) {
  ...
  return (
    <div className="flex h-[min(720px,calc(100vh-8rem))] flex-col overflow-hidden rounded-xl border border-chat-border bg-chat-bg shadow-sm">
      <div className="flex items-center justify-between border-b border-chat-border px-4 py-3">
        <div className="flex items-center gap-2">
          <span
            ...
            aria-hidden="true"
          />
          <span className="text-sm font-medium text-neutral-900 dark:text-neutral-100">Agent</span>
      ...
      >
        {messages.length === 0 && !isLoading ? (
          ...
          <>
            ...
            {error && (
              <div role="alert" className="mt-2 rounded-lg border border-red-200 bg-red-50 px-3 py-2 text-xs text-red-800 dark:border-red-900 dark:bg-red-950 dark:text-red-200">
                {error}
              </div>
            )}
            <div ref={bottomRef} />
